...
function getTab(tabId){
    ...
    var rows = $(""+tabId+" .testCase").length;
        if($(""+tabId+" .testCase").css("display") == "none"){
                    $(""+tabId+" .testCase").css({"display":"table-row"})
             }
        $(""+tabId+" .testCase:gt(9)").hide();
        var currtPage = 1;//当前页
        var pagesize = 10;//显示条数
        var totalpage = Math.ceil(rows/pagesize);
        var pagesizes = pagesize-1;
        Page({
                num:totalpage,					//页码数
                startnum:currtPage,				//指定页码
                elem:$('#page2'),		//指定的元素
                callback:function(n){	//回调函数
                    console.log(n);
                    $.each($(""+tabId+" .testCase"),function(i){
                        var start = pagesize*(n-1);
                        var end = pagesize*n;
                        if(i>=start&&i<end){
                            $(this).show();
                        }else{
